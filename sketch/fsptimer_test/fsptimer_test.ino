...
void setup() {
  ...
  gpt4_ext_cfg->gtioca.output_enabled = true; // not referenced when gtior is not 0
  gpt4_ext_cfg->gtiocb.output_enabled = true; // not referenced when gtior is not 0
  ...
  gpt4_ext_pwm_cfg->dead_time_count_up = 120; // dead time 120 clock cycles
...
void loop() {
  ...
  gpt4.set_duty_cycle(100 + i * 10, CHANNEL_A);
  gpt4.set_duty_cycle(100 + i * 10, CHANNEL_B);
  i++;
